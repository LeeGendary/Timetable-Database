...
app.post("/expenses", async (req, res) => {
  try {
    ...
    const newExpenses = await db.query(
      "insert into expenses (name,type,amount,date) values($1,$2,$3,$4) returning * ",
...
app.get("/expenses/current", async (req, res) => {
  try {
    const currentMonth = await db.query(
      `select * from expenses where date >= date_trunc('month',CURRENT_DATE);`
    )
...
app.get("/expenses/monthly/:month", async (req, res) => {
  try {
    ...
    const allExpensesByMonth = await db.query(
      `select * from expenses where extract(month from date)=$1 order by date desc;`,
      [month]
    )
...
app.get("/expenses/:id", async (req, res) => {
  try {
    const { id } = req.params
    const expense = await db.query(`select * from expenses where id = $1 ;`, [
      id,
    ])
...
app.put("/expenses/:id", async (req, res) => {
  try {
    ...
    const updateToDo = await db.query(
      "update expenses set name = $1 ,type = $2 ,amount = $3, date = $4 where id = $5;",
...
app.delete("/expenses/:id", async (req, res) => {
  try {
    ...
    const deleteExpense = await db.query(
...
app.post("/incomes", async (req, res) => {
  try {
    ...
    const newIncome = await db.query(
      "insert into incomes (channel,amount,date) values($1,$2,$3) returning * ",
      [channel, amount, date]
    )
...
app.get("/incomes", async (req, res) => {
  try {
    const allIncomes = await db.query(
...
app.get("/incomes/monthly/:month", async (req, res) => {
  try {
    ...
    const allIncomesByMonth = await db.query(
      `select * from incomes where extract(month from date)=$1 order by date desc;`,
      [month]
    )
...
app.get("/incomes/categories", async (req, res) => {
  try {
    const categories = await db.query(
      'select channel, SUM(amount) as "total" from incomes group by type order by total desc;'
    )
    res.json(categories.rows)
  } catch (error) {
    console.error(error.message)
...
app.get("/incomes/:id", async (req, res) => {
  try {
    const { id } = req.params
    const expense = await db.query(`select * from incomes where id = $1 ;`, [
      id,
    ])
    res.json(expense.rows[0])
  } catch (error) {
...
app.put("/incomes/:id", async (req, res) => {
  try {
    const { id } = req.params
    const { channel, amount, date } = req.body
    const updateIncome = await db.query(
      "update incomes set channel = $1 ,amount = $2, date = $3 where id = $4;",
      [channel, amount, date, id]
    )
...
app.delete("/incomes/:id", async (req, res) => {
  try {
    ...
    const deleteExpense = await db.query("delete from incomes where id = $1;", [
      id,
    ])
...
app.put("/budget/:id", (req, res) => {
  ...
  db.query(
    "update budget set budget_amount = $1 where id = $2;",
    [budget_amount, id],
    (err, dbRes) => {
      res.json("updated")
    }
...
app.get("/goals", (req, res) => {
  db.query("select * from goals", (err, dbRes) => {
    res.json(dbRes.rows)
  })
})
...
app.get("/goals/:id", (req, res) => {
  const { id } = req.params
  // const {imageurl,title,amount,date} = req.body
  db.query("select * from goals where id = $1;", [id], (err, dbRes) => {
    res.json(dbRes.rows)
  })
})
...
app.post("/goals", async (req, res) => {
  try {
    const { imageurl, title, amount, date } = req.body
    const newGoals = await db.query(
      "insert into goals (imageurl,title,amount,date) values($1,$2,$3,$4) returning * ",
      [imageurl, title, amount, date]
    )

    res.json(newGoals.rows[0])
  } catch (error) {
    console.error(error.message)
  }
})
